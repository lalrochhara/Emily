...
from Emily.__main__ import STATS, TOKEN, USER_INFO
import Emily.modules.sql.userinfo_sql as sql
from Emily.modules.disable import DisableAbleCommandHandler
from Emily.modules.sql1.global_bans_sql import is_user_gbanned
from Emily.modules.sql1.afk_sql import is_afk, check_afk_status
from Emily.modules.sql.users_sql import get_user_num_chats
from Emily.modules.helper_funcs.chat_status import sudo_plus
from Emily.modules.helper_funcs.extraction import extract_user
...
@tbot.on(
    events.NewMessage(
        pattern="/ginfo ", from_users=(TIGERS or []) + (DRAGONS or []) + (DEMONS or []) + (DEV_USERS or [])
    )
)
...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/Emily_logss/2">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
