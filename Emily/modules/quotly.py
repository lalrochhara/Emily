...
async def quo(_, message):
    ...
    user = (
        await pbot.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, canvas = await process(msg, user, pbot, reply, repliedreply)
    if not res:
        return
    canvas.save("sticker.webp")
    await pbot.send_file(
        message.chat_id, "sticker.webp", reply_to=message.reply_to_msg_id
    )
