from Emily.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Emily import DRAGONS
...
async def Emily_is_admin(chat_id: int):
    status = False
    Emily = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Emily.id == user.id:
